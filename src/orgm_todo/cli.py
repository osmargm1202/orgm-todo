...
app.add_typer(proyecto_app, name="proyecto")
...
def client_create(
    nombre: str,
    dato: Annotated[list[str], typer.Option("--dato")] = [],
    empresa: Annotated[str | None, typer.Option()] = None,
    contacto: Annotated[str | None, typer.Option()] = None,
    telefono: Annotated[str | None, typer.Option()] = None,
    correo: Annotated[str | None, typer.Option()] = None,
    direccion: Annotated[str | None, typer.Option()] = None,
) -> None:
...
@cliente_app.command("actualizar")
def client_update(
    nombre: str,
    dato: Annotated[list[str], typer.Option("--dato")] = [],
    quitar_dato: Annotated[list[str], typer.Option("--quitar-dato")] = [],
) -> None:
...
@proyecto_app.command("restaurar")
...
@titulo_app.command("agregar")
def title_add(titulo: str, proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    run(lambda: vault().add_title(titulo, proyecto))


@titulo_app.command("renombrar")
def title_rename(actual: str, nuevo: str, proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    run(lambda: vault().rename_title(actual, nuevo, proyecto))


@titulo_app.command("eliminar")
def title_delete(titulo: str, proyecto: Annotated[str | None, typer.Option()] = None, force: bool = False) -> None:
    run(lambda: vault().delete_title(titulo, proyecto, force))


@nota_app.command("agregar")
def note_add(texto: str, titulo: Annotated[str, typer.Option()] = "Pendiente", proyecto: Annotated[str | None, typer.Option()] = None) -> None:
...
@nota_app.command("listar")
def note_list(proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    print_entries(run(lambda: vault().list_entries(proyecto, False)))


@nota_app.command("actualizar")
def note_update(selector: str, texto: Annotated[str, typer.Option()], proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    run(lambda: vault().update_note(selector, texto, proyecto))


@nota_app.command("mover")
def note_move(selector: str, titulo: Annotated[str, typer.Option()], proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    run(lambda: vault().move_item(selector, titulo, proyecto, False))


@nota_app.command("eliminar")
def note_delete(selector: str, proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    run(lambda: vault().delete_item(selector, proyecto, False))


@tarea_app.command("agregar")
def task_add(
    texto: str,
    titulo: Annotated[str, typer.Option()] = "Pendiente",
    proyecto: Annotated[str | None, typer.Option()] = None,
    fecha: Annotated[str | None, typer.Option()] = None,
    cada: Annotated[str | None, typer.Option()] = None,
    desde: Annotated[str | None, typer.Option()] = None,
    hasta: Annotated[str | None, typer.Option()] = None,
) -> None:
    def action():
        store = vault()
        if cada:
            if fecha or not desde or not hasta:
                raise VaultError("--cada exige --desde y --hasta; no se combina con --fecha")
            return store.recurring_tasks(texto, cada, date.fromisoformat(desde), date.fromisoformat(hasta), titulo, proyecto)
        if desde or hasta:
            raise VaultError("--desde y --hasta requieren --cada")
        return [store.add_task(texto, titulo, proyecto, fecha)]
    for ident in run(action):
        console.print(ident)


@tarea_app.command("listar")
def task_list(proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    print_entries(run(lambda: vault().list_entries(proyecto, True)))


@tarea_app.command("completar")
def task_complete(selector: str, proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    run(lambda: vault().set_task_state(selector, True, proyecto))


@tarea_app.command("pendiente")
def task_pending(selector: str, proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    run(lambda: vault().set_task_state(selector, False, proyecto))


@tarea_app.command("actualizar")
def task_update(
    selector: str,
    proyecto: Annotated[str | None, typer.Option()] = None,
    texto: Annotated[str | None, typer.Option()] = None,
    fecha: Annotated[str | None, typer.Option()] = None,
    sin_fecha: Annotated[bool, typer.Option()] = False,
) -> None:
    if fecha and sin_fecha:
        fail(VaultError("--fecha y --sin-fecha no se pueden combinar"))
...
@tarea_app.command("mover")
def task_move(selector: str, titulo: Annotated[str, typer.Option()], proyecto: Annotated[str | None, typer.Option()] = None) -> None:
    run(lambda: vault().move_item(selector, titulo, proyecto, True))


@tarea_app.command("eliminar")
def task_delete(selector: str, proyecto: Annotated[str | None, typer.Option()] = None) -> None:
...
def period(hoy: bool, semana: bool, mes: str | None, desde: str | None, hasta: str | None) -> tuple[date | None, date | None, str]:
    ...
    if chosen > 1 or (desde is None) != (hasta is None):
        raise VaultError("Use solo un filtro temporal; --desde requiere --hasta")
    now = date.today()
    if hoy:
    ...
    if desde:
        ...
        if end < start:
            raise VaultError("--hasta no puede ser anterior a --desde")
...
@app.command("resumen")
def summary(
    titulo: Annotated[str | None, typer.Option()] = None,
    proyecto: Annotated[str | None, typer.Option()] = None,
    incluir_hechas: Annotated[bool, typer.Option()] = False,
    incluir_sin_fecha: Annotated[bool, typer.Option()] = False,
    hoy: Annotated[bool, typer.Option()] = False,
    semana: Annotated[bool, typer.Option()] = False,
    mes: Annotated[str | None, typer.Option()] = None,
    desde: Annotated[str | None, typer.Option()] = None,
    hasta: Annotated[str | None, typer.Option()] = None,
) -> None:
    start, end, label = run(lambda: period(hoy, semana, mes, desde, hasta))
    entries: list[SummaryEntry] = run(lambda: vault().summaries(titulo, proyecto, incluir_hechas, incluir_sin_fecha, start, end))
